...
const Navigation = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm" style={{ height: 'var(--header-h)' }}>
      <div className="container-page">
        <div className="flex justify-between items-center" style={{ height: 'var(--header-h)' }}>
          ...
          {/* Desktop Menu */}
          <div className="hidden md:flex gap-flow-sm">
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center gap-2 p-card-sm rounded-md transition-colors whitespace-nowrap ${
                  isActive(item.path) 
            ...
            <NavLink
              to="/3d"
              className={({ isActive }) =>
                `flex items-center gap-2 p-card-sm rounded-md transition whitespace-nowrap ${
                  isActive 
        ...
        {/* Mobile Menu */}
        {isOpen && (
          <div className="md:hidden absolute left-0 w-full bg-white border-b border-gray-200 z-40 shadow-lg" style={{ top: 'var(--header-h)' }}>
            <div className="flex flex-col py-gutter gap-flow-sm">
              {navItems.map((item) => (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex items-center gap-3 px-gutter py-card-sm transition-colors ${
                    isActive(item.path) 
                      ? "text-blue-600 font-medium bg-blue-50" 
              ...
              <NavLink
                to="/3d"
                className={({ isActive }) =>
                  `flex items-center gap-3 px-gutter py-card-sm rounded-md transition ${
                    isActive 
                      ? "bg-blue-500 text-white font-bold" 
